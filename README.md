...
| **Orange** | Enterprise-Charcoal: neutrales Dunkelgrau, kräftiges Orange als Primärfarbe, reduzierte Radien |
...
| IT-Monitoring | **IT-Dashboard** (`dashboard.html` + `data.js`): Status-Matrix mit Uptime-Badge + Latenz-Sparkline pro Service · Metric Cards mit SVG-Sparklines + Threshold-Balken · Log-Viewer mit Level-/Volltext-Filter und Pagination · Inventar-Grid · Wartungsfenster · Incident-Timeline mit proportionalem Dauer-Balken und MTTR · **Zertifikatsüberwachung** (TLS-Ablaufdaten mit Ampel, optionales Live-Prüfen via Proxy, 📜-Button je Zeile zum Prefill des CSR-Generators) · Tab-Titel-Ampel (❌/⚠/✅) · Auto-Refresh (HTTPS) |
| Formulare & Tabellen | **Formular-System** (`.form-section`/`.form-grid`/`.form-group`/`.radio-group` — 2-spaltiger Grid, Card-Abschnitte, Label+Input, Aktionszeile) · **Datentabelle read-only** (`.tbl-wrap`/`.tbl` — leichtgewichtig, `td.mono` für IPs/UUIDs, `td.actions` für Button-Zellen; ergänzt `.rt` und `.edt-table`) · **Bulk-Aktions-Leiste** (`.bulk-bar` fixed bottom — erscheint wenn ≥1 Zeile per Checkbox ausgewählt; `Set`-basierter Selection-State mit `bulkToggle`/`bulkToggleAll`/`bulkClear`) · **Leer-Zustand** (`.empty`/`.ei` — zentrierter Emoji + Text für Sektionen ohne Inhalt) |
| API-Integration | **API-Connector** (OAuth2 Client Credentials, Bearer-Token im sessionStorage, clientSecret nie persistiert) · **Toast-Notifications** (Statusmeldungen ok/err/info, auto-dismiss) · **Confirmation-Dialog** (`showConfirm({icon, title, body, okLabel, okClass, callback})` — modales Overlay für destruktive Aktionen, `confirmDialog(msg,cb)` als rückwärtskompatibles Alias) · **Editierbare Datentabelle** (Paginierung, Suche, Inline-Edit mit PATCH) · **Multi-Step-Wizard** (Schritt-Indikator, hier: CSR-Generator mit PEM-Download + Prefill aus PEM-Datei oder Zertifikatsüberwachungstabelle) · **SSH-Backup-Manager** (Backup-Liste über SSH-Proxy, Restore mit Bestätigung) |
