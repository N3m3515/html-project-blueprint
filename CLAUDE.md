...
| `data.js` | Daten-Datei des IT-Dashboard-Bausteins (`var DASH_DATA = {...}`) — Felder: `services[]` (inkl. `uptime_pct`, `latency_history`), `metrics[]`, `logs[]`, `maintenance[]`, `incidents[]`, `hosts[]`; per Monitoring-Script erzeugen; funktioniert bei `file://` |
...
| **API-Connector** (Dashboard) | OAuth2-Client-Credentials-Flow für REST-APIs (ClearPass, andere); zentraler `apiFetch(method,path,body)` Helper; Token im sessionStorage mit Ablaufzeit | `<section id="api-config">` mit `.api-form`-Feldern, CSS `/* === Baustein: API-Connector === */`, `apiLogin()`/`apiLogout()`/`apiFetch()`/`updateApiStatus()` + Init-IIFE im API-Connector-Script-Block; **Sicherheit: clientSecret NIEMALS persistieren** — nur `aruba_base_url` + `aruba_client_id` + `aruba_token` im sessionStorage | Sektion + Script-Block weglassen; `apiFetch`-Aufrufe in abhängigen Bausteinen ebenfalls entfernen |
| **Editierbare Datentabelle** (Dashboard) | Datentabelle mit Paginierung, Volltextsuche und Inline-Bearbeitung (Attribute) — PATCH pro Zeile | `<section id="clearpass-endpoints">` mit `.edt-wrap`/`.edt-table`, CSS `/* === Baustein: Editierbare Datentabelle === */`, `loadEndpoints()`/`renderEndpointTable()`/`saveEndpoint(btn)`/`epPrevPage()`/`epNextPage()` + Init-IIFE im Script-Block; setzt **API-Connector** voraus | Sektion + Script-Teil weglassen; API-Pfad `/api/endpoint` auf das eigene System anpassen |
| **Multi-Step-Wizard** (Dashboard) | Drei-Schritt-Wizard mit Schritt-Indikator — hier: CSR-Generator (Service wählen → Felder füllen → PEM herunterladen) | `<section id="csr-generator">` mit `.wiz-indicator`/`.wiz-step`-Markup, CSS `/* === Baustein: Multi-Step-Wizard === */`, `csrSetStep(n)`/`csrNext1()`/`csrGenerate()`/`csrDownload()` + Init-IIFEs im Script-Block; setzt **API-Connector** voraus; Download via Blob-URL | Sektion + Script-Teil weglassen; Schritt-Texte + API-Pfad anpassen |
| **NetEdit Backup Manager** (Dashboard) | Liste + Erstellen + Restore von NetEdit-Backups über einen SSH-Proxy; Restore mit Confirmation-Dialog | `<section id="netedit-backups">` mit `.backup-list`, CSS `/* === Baustein: NetEdit Backup Manager === */`, `loadNetEditBackups()`/`createNetEditBackup()`/`renderBackupList()`/`restoreBackup(name)` im Script-Block; setzt **Confirmation-Dialog** voraus; Proxy-URL aus sessionStorage (`netedit_proxy_url`); **kein eigenes Auth** — Proxy übernimmt SSH | Sektion + Script-Teil weglassen; Proxy-Endpunkte `GET /backups`, `POST /backup`, `POST /restore` an eigenes System anpassen |
...
- Baustein Multi-Step-Wizard: `.wiz-indicator` > `.wi-col`(`.wi-dot`(`.wi-active`/`.wi-done`)+`.wi-label`) + `.wi-line` · `.wiz-step`(`.wiz-active`) · `.wiz-fields`(Labels+Inputs) · `.wiz-csr-out` · `.wiz-nav`
...
- Baustein NetEdit Backup Manager: `.backup-list` > `.backup-item`(`.backup-name`+`.backup-date`+Button)
