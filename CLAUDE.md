...
| **Zertifikatsüberwachung** (Dashboard) | Ablaufdaten von TLS-Zertifikaten als Tabelle mit Ampel-Färbung; optionales Live-Prüfen via Proxy | `<section id="cert-monitor">` mit `.cert-table-wrap`/`.cert-table`, CSS `/* === Baustein: Zertifikatsüberwachung === */`, Rendering-Code in IT-Dashboard-IIFE (`D.certificates[]`), `checkCert(btn)` global; Proxy-Endpunkt `GET /cert-check?host=HOST&port=PORT` → `{days_remaining:N}`; Schwellwerte: crit < 14 Tage, warn < 30 Tage | Sektion + Rendering-Code + `checkCert` weglassen; `certificates`-Feld in data.js bleibt harmlos |
