...
| `data.js` | Daten-Datei des IT-Dashboard-Bausteins (`var DASH_DATA = {...}`) — per Monitoring-Script erzeugen; funktioniert bei `file://` |
...
| **IT-Dashboard** | Infrastruktur-Status auf einer Seite — Status-Matrix (Ampel-Grid), Metric Cards + SVG-Sparklines, Log-Viewer mit Volltext-/Level-Filter, Inventar-Grid mit Status-Farbrand | `dashboard.html` als Template, `data.js` als Datendatei (`var DASH_DATA = {...}`) — `<script src="data.js">` lädt die Daten, funktioniert auch bei `file://`; CSS-Marker `/* === Baustein: IT-Dashboard — Status-Matrix === */`, `/* === … — Metriken === */`, `/* === … — Log-Viewer === */`, `/* === … — Inventar === */`; Script-Block 6 rendert alle vier Bereiche per JS aus `DASH_DATA`; Stale-Indikator wenn Daten > 10 Min. alt; Auto-Refresh-Checkbox (nur HTTPS) | `dashboard.html` + `data.js` weglassen; alle vier CSS-Baustein-Blöcke bleiben harmlos in anderen Seiten (kein Markup = kein Effekt); `PRECACHE` in `sw.js` entsprechend kürzen |
